...
function toPreviewBlock(editor: Editable): CompositionDataBlock {
  const display = editor.configuration.display === 'metric' ? 'metric' : editor.configuration.display === 'text' ? 'text' : editor.configuration.display === 'table' ? 'list' : editor.configuration.display === 'matrix' ? 'matrix' : editor.configuration.display === 'timeline' ? 'timeline' : 'trend';
  return {
  ...
  return <div style={{ padding: '1rem', minHeight: 'calc(100vh - 2rem)', boxSizing: 'border-box' }}>
    ...
    <div className="card" style={{ display: 'grid', gridTemplateColumns: '245px minmax(440px, 1fr) 315px', minHeight: 'calc(100vh - 145px)', padding: 0 }}>
      ...
      <main style={{ padding: '1.15rem', overflow: 'auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '.7rem' }}><label className="form-label">Darstellung<select className="form-input" value={editor.configuration.display} onChange={(event) => config({ display: event.target.value as Display })}><option value="line">Linie</option><option value="area">Fläche</option><option value="bar">Balken</option><option value="metric">Kennzahl</option><option value="table">Tabelle</option><option value="text">Text</option><option value="matrix">Matrix (Parameter × Zeit)</option><option value="timeline">Zeitleiste</option></select></label><label className="form-label">Max. Zeilen<input className="form-input" type="number" min="1" max="1000" value={editor.configuration.limit || 100} onChange={(event) => config({ limit: Number(event.target.value) || 100 })} /></label></div>
