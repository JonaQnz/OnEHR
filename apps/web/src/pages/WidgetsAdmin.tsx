...
type Display = 'line' | 'area' | 'bar' | 'metric' | 'table' | 'text' | 'matrix';
...
function toPreviewBlock(editor: Editable): CompositionDataBlock {
  const display = editor.configuration.display === 'metric' ? 'metric' : editor.configuration.display === 'text' ? 'text' : editor.configuration.display === 'table' ? 'list' : editor.configuration.display === 'matrix' ? 'matrix' : 'trend';
  return {
