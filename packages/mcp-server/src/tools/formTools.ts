...
export function registerFormTools(server: McpServer, api: FormbuilderApiClient): void {
  ...
  server.registerTool('audit_form_bindings', {
    ...
    description: 'Checks a Form Section\'s stored bindings against the CURRENT state of its source template, fetched live from EHRbase (not the possibly-stale local import cache - re-import first with import_remote_template if you want the audit itself to reflect a template change you just made on EHRbase). A binding is a snapshot taken when the form was built or last regenerated; it never updates itself. Flags: unresolved-path (the archetype path no longer exists in the template - re-versioned or restructured), rmtype-mismatch (the RM type at that path changed), stale-option (a stored DV_CODED_TEXT/CODE_PHRASE option is no longer a valid code - EHRbase would reject a submission that picks it), stale-unit (a stored DV_QUANTITY unit is no longer offered by the current template), and missing-quantity-constraint (the current template specifies a magnitude min/max/precision for a unit this field uses that its own stored unitOptions doesn\'t carry yet - re-apply the template to pick it up; only ever a soft nudge, since the field wasn\'t wrong before this existed, just incomplete). Read-only - never modifies the form or the template. Deliberately does not flag "should this now be a repeatable group" - apply_template_to_form is the authoritative fix for that class of drift, not this audit.',
