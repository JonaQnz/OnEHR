...
The example n8n plugin contributes a form-settings action and registers itself as a real `FormDataProvider` (via `registerFormDataProvider` - see below), reachable as provider id `n8n` only while this plugin is loaded. Configure `N8N_API_URL`, `N8N_API_KEY`, and `N8N_PUBLIC_URL`, load `formbuilder-example-n8n-plugin`, open a form's `Submission` settings, and click `Als n8n Form konfigurieren`. The plugin creates or updates a bare `Webhook → protocol-adapter` workflow per enabled lifecycle hook and stores only the neutral workflow reference in the form definition - any actual clinical logic (e.g. calling EHRbase) is left for the n8n workflow author to build in nodes before that adapter, not auto-generated. Form runtime submissions then POST the complete `formbuilder.form-submission.v1` payload to the submit webhook.

A plugin that supplies a data provider registers the live implementation, not just descriptive metadata: `context.registerFormDataProvider(provider)` (a real `FormDataProvider` with `load`/`submit`) makes it reachable via the host's `getDataProvider(id)`, and derives the equivalent `registerDataProvider` metadata contribution automatically. Use the metadata-only `registerDataProvider` alone only when a plugin wants to advertise a provider capability without actually being the one to implement it.
