...
function getUIFieldTypeLabel(element) {
  if (element === 'TextInput') return 'Text Input';
  if (element === 'NumberInput') return 'Number Input';
  if (element === 'Dropdown') return 'Dropdown';
  if (element === 'DatePicker') return 'Date Picker';
  if (element === 'Checkboxes') return 'Checkbox';
  if (element === 'RadioButtons') return 'Radio Buttons';
  if (element === 'TextArea') return 'Text Area';
  if (element === 'Paragraph') return 'Paragraph';
  if (element === 'Header') return 'Header';
  if (element === 'LineBreak') return 'Divider';
  if (element === 'Rating') return 'Star Rating';
  if (element === 'Range') return 'Slider / Range';
  if (element === 'Tags') return 'Tags Input';
  return element || '';
}

function FieldStatusBar({ data, mutable }) {
  const [expanded, setExpanded] = React.useState(false);

  if (mutable) return null; // don't show in live form mode
  const binding = data?.custom_metadata?.binding;
  if (!binding) return null;
  const shortPath = binding.path
    ? binding.path.split('/').slice(-3).join('/')
    : '';

  const uiType = getUIFieldTypeLabel(data.element);

  if (!expanded) {
    return (
      <div className="rfb-field-status collapsed">
        <div className="status-left">
          <span className="status-ui-label">{uiType}</span>
        </div>
        <div className="status-right">
          <span className="status-rmtype">{binding.rmType}</span>
          <span className="status-sep">·</span>
          <span className="status-path" title={binding.path}>…/{shortPath}</span>
          <button 
  ...
  return (
    <div className="rfb-field-status expanded">
      <div className="status-expanded-sections">
        {/* Form Properties */}
        <div className="expanded-section form-props">
          <div className="expanded-section-header">Form Properties</div>
          <div className="status-grid">
            <div className="status-grid-row">
              <span className="grid-label">Field ID:</span>
              <span className="grid-value status-name">{data.field_name?.replace(/_$/, '') || ''}</span>
            </div>
            <div className="status-grid-row">
              <span className="grid-label">Field Type:</span>
              <span className="grid-value status-name">{uiType}</span>
            </div>
          </div>
        </div>

        {/* openEHR Properties */}
        <div className="expanded-section openehr-props">
          <div className="expanded-section-header">openEHR Properties</div>
          <div className="status-grid">
            <div className="status-grid-row">
              <span className="grid-label">RM Type:</span>
              <span className="grid-value status-rmtype-expanded">{binding.rmType}</span>
            </div>
            <div className="status-grid-row">
              <span className="grid-label">AQL Path:</span>
              <span className="grid-value status-path-expanded" title={binding.path}>{binding.path}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <button 
        className="btn-status-toggle-hide" 
        type="button"
  ...
  render() {
    ...
    return (
      <div style={{ ...this.props.style }} className={baseClasses}>
        ...
        <FieldStatusBar data={this.props.data} mutable={this.props.mutable} />
      </div>
    );
  }
}
...
class NumberInput extends React.Component {
  ...
  render() {
    ...
    const meta = this.props.data.custom_metadata || {};
    const unitOptions = meta.unitOptions || [];
    const isQuantity = meta.binding?.rmType === 'DV_QUANTITY';
            ...
            {unitOptions.length > 1 && (
              <select className="form-control" style={{ width: 'auto' }}>
                {unitOptions.map((opt, i) => (
                  <option key={i} value={opt.unit}>{opt.unit}</option>
                ))}
              </select>
            )}
          </div>
          
          {unitOptions.length === 0 && isQuantity && (
            <div style={{ fontSize: '0.8rem', color: '#ef4444', marginTop: '0.25rem' }}>
              ⚠ Für "{this.props.data.label}" wurde keine erlaubte Einheit gefunden. Bitte Template-Constraint prüfen.
            </div>
          )}
